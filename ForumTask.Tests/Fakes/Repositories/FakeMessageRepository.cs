...
{
    internal class FakeMessageRepository : FakeGenericRepository<Message>, IMessageRepository
    {
        public FakeMessageRepository(params Message[] data) : base(data) { }

        public int GetMessageCount(long topicId)
        {
            return data.Count(m => m.TopicId == topicId);
        }

        public IEnumerable<Message> GetTopOld(long topicId, int count, int offset)
        {
            return data.Where(m => m.TopicId == topicId).Skip(offset).Take(count);
        }
